...
const InputSection: React.FC<Props> = ({ onAnalyze, isLoading }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      const processed = await Promise.all(allowedFiles.map((file: File) => {
        ...
        <div className="bg-white rounded-[2rem] md:rounded-[3rem] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.06)] border border-slate-100 p-6 md:p-12 space-y-8 md:space-y-10">
          ...
          {files.length > 0 && (
            <div className="flex flex-wrap gap-3 pt-4">
              {files.map((f, i) => {
                ...
                return (
                  <div key={i} className={`flex items-center gap-2 px-3 py-2 rounded-xl border group transition-all animate-in zoom-in-95 ${isPdf ? 'bg-green-50 border-green-100 text-green-600' : 'bg-slate-50 border-slate-100 text-slate-500'
                    }`}>
                    {isPdf ? <FileCheck size={14} /> : <FileText size={14} />}
                    <span className="text-[10px] font-black truncate max-w-[120px]">{f.file.name}</span>
                    {isPdf && <span className="text-[8px] bg-green-500 text-white px-1.5 py-0.5 rounded font-black tracking-tighter">HQ</span>}
                    <button onClick={() => removeFile(i)} className="hover:text-rose-500 transition-colors">
                      <X size={14} />
                    </button>
                  </div>
                );
          ...
          <div className="bg-blue-50/30 rounded-2xl p-4 md:p-5 border border-blue-50/50 flex items-start gap-4">
            ...
            <p className="text-[10px] text-slate-500 leading-relaxed font-bold tracking-tight">
              AIは入力されたすべてのソースを読み込み、矛盾を解消しながら「あなた」という人物像を再構築します。<br className="hidden md:block" />
              <span className="text-blue-600 font-black underline decoration-blue-200">PDF形式の資料</span>を混ぜると、分析の根拠が強まり精度が向上します。
            </p>
          </div>
          ...
          <button
            ...
            className={`w-full py-4 md:py-5 rounded-full font-black text-base md:text-lg tracking-[0.2em] uppercase flex items-center justify-center gap-3 transition-all ${isLoading || !hasInput
